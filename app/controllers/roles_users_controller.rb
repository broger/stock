class RolesUsersController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @roles_user.save
        format.html { redirect_to(@roles_user, :notice => 'RolesUser was successfully created.') }
  ...
  def update
    ...
    respond_to do |format|
      if @roles_user.update_attributes(params[:roles_user])
        format.html { redirect_to(@roles_user, :notice => 'RolesUser was successfully updated.') }
        format.xml  { head :ok }
      else
